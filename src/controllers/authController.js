...
const loginAdmin = async (req, res, next) => {
    ...
    try {
        const [rows] = await db.query('SELECT * FROM administradores WHERE usuario = ? AND activo = 1', [usuario]);
...
const loginCliente = async (req, res, next) => {
    ...
    try {
        const [rows] = await db.query('SELECT * FROM clientes WHERE usuario = ? AND activo = 1', [usuario]);
